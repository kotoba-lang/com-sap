(ns sap.main
  "Kotodama WASM entrypoint for the Sap clean-room actor (L4) — Clojure port.
  ...
  facts (`sap.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"sap.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sap-compat" :tier tier :entities entities} 200])
